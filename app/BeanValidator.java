...
public class BeanValidator {
	...
	// More strict mapper for validation. Makes sure input does not have anything
	// after a
	...
	public void validate(Object bean, String fieldNamePrefix) {
		/*
		 * TODO It will not convert play constraints message codes (like
		 * `error.required`) to correct messages. Need work to make it work OR use
		 * javax.validation or hibernate validation annotations - which work out of the
		 * box.
		 */
		Map<String, List<String>> validationErrors = validator.validate(bean).stream()
				.collect(Collectors.groupingBy(e -> getFieldName(e.getPropertyPath().toString(), fieldNamePrefix),
