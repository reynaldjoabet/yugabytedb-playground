...
public class OidcTokenValidator {
	...
	public OidcTokenValidator(String oidcDiscoveryUrl, List<String> clientIds) throws IOException {
		...
		clientIds
				.forEach((id) -> validators.put(id, createValidator(config.issuer, config.jwksUri, JWS_ALGORITHM, id)));
	}
	...
	public IDTokenClaimsSet validate(JWT idToken) throws BadJOSEException, JOSEException, ParseException {
		...
		/*
		 * Tokens that have more than 1 audience should also have the AZP claim. Nimbus
		 * will check that tokens with multiple audiences will have an AZP claim that
		 * matches one of the audiences.
		 */
	...
	private IDTokenValidator createValidator(String issuerUrl, String jwksUrl, JWSAlgorithm algorithm,
			String clientId) {
