...
public class RoleBinding extends Model {
  ...
  public static RoleBinding getOrBadRequest(UUID roleBindingUUID) {
    RoleBinding roleBinding = null;//get(roleBindingUUID);
    if (roleBinding == null) {
  ...
  public static List<RoleBinding> getAll() {
    List<RoleBinding> list = null;//find.query().findList();
  ...
  public static List<RoleBinding> getAllWithRole(UUID roleUUID) {
    List<RoleBinding> rbList = null;//find.query().where().eq("role_uuid", roleUUID).findList();
    return rbList;
  }
