...
public final class Argon2idHash extends Hash {
	...
	private static final Pattern HASHSTRING_PARAM_PATTERN = Pattern
			.compile("^(m=(?<memory>[0-9]+)|t=(?<iterations>[0-9+])|p=(?<parallelism>[0-9]+))$");
	...
	public void digest(String[] inputs) {
		...
		Argon2Parameters parameters = new Argon2Parameters.Builder(Argon2Parameters.ARGON2_id)
				.withVersion(Argon2Parameters.ARGON2_VERSION_13).withIterations(this.iterations)
				.withMemoryAsKB(this.memory).withParallelism(this.parallelism).withSalt(this.salt).build();
	...
	public String toHashString() {
		...
		return new StringBuffer().append("$").append("argon2id").append("$")
				.append(String.format("v=%d", Argon2Parameters.ARGON2_VERSION_13)).append("$")
				.append(String.format("m=%d,t=%d,p=%d", memory, iterations, parallelism)).append("$")
				.append(encodedSalt).append("$").append(encodedHash).toString();
	}
