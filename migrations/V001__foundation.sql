...
BEGIN
  IF TG_OP = 'INSERT' THEN
    ...
    INSERT INTO audit.audit_event(tenant_id, action, entity_schema, entity_table, entity_key, after_row)
    VALUES (v_tenant_id, TG_OP, TG_TABLE_SCHEMA, TG_TABLE_NAME, v_entity_key, to_jsonb(NEW));
    RETURN NEW;
  ELSIF TG_OP = 'UPDATE' THEN
    v_tenant_id := CASE WHEN to_jsonb(NEW) ? 'tenant_id' THEN (to_jsonb(NEW)->>'tenant_id')::uuid ELSE NULL END;
    v_entity_key := jsonb_build_object('op', TG_OP, 'old', to_jsonb(OLD), 'new', to_jsonb(NEW));
    INSERT INTO audit.audit_event(tenant_id, action, entity_schema, entity_table, entity_key, before_row, after_row)
    VALUES (v_tenant_id, TG_OP, TG_TABLE_SCHEMA, TG_TABLE_NAME, v_entity_key, to_jsonb(OLD), to_jsonb(NEW));
    RETURN NEW;
  ELSIF TG_OP = 'DELETE' THEN
    ...
    INSERT INTO audit.audit_event(tenant_id, action, entity_schema, entity_table, entity_key, before_row)
    VALUES (v_tenant_id, TG_OP, TG_TABLE_SCHEMA, TG_TABLE_NAME, v_entity_key, to_jsonb(OLD));
    RETURN OLD;
  END IF;
